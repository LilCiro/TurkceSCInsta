    ...
    steps: # İşin adımları. 👣➡️
      ...
      - name: SCInsta tweak'i sideload için derle (IPA olarak) 🏗️📱🚀📦
        run: |
          ...
          # Uygulama adı, geliştirici adı ve ikon kaldırma argümanlarını hazırla
          ...
          if [[ "${{ inputs.remove_app_icon }}" == "true" ]]; then
            BUILD_SCRIPT_ARGS+=" --remove-app-icon" # build.sh betiğinizde bu argümanı işlemeniz gerekecek
          fi

          ./build.sh $BUILD_SCRIPT_ARGS # Hazırlanan argümanlarla derleme betiğini çalıştırır.
